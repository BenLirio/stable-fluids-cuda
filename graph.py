...
from sfc import USE_SHARED_MEMORY

def graph_shared_memory_functions(timings):
  # Mostly generated by ChatGPT
  fields = ['ADVECT', 'DIFFUSE', 'PROJECT']
  categories = ['Naive', 'Shared Memory']
  data = {
    'NAIVE': {
      'ADVECT': [],
      'DIFFUSE': [],
  ...
  naive = data['NAIVE']
  totals = [
    [ sum(naive['ADVECT']), sum(naive['DIFFUSE']), sum(naive['PROJECT']) ],
    [ sum(shared_memory['ADVECT']), sum(shared_memory['DIFFUSE']), sum(shared_memory['PROJECT']) ],
  ]
  ...
  ax.set_title('Shared Memory Compare Functions')
  ...
  fig.savefig('timings/share_memory_functions.png')
